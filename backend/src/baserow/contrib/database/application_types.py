    ...
    def _import_table_views(
        ...
        for serialized_view in serialized_table["views"]:
            try:
                ...
                # View types are contributed by plugins, so an export can name
                # one this instance does not have. The rows are what matter, so
                # skip the view rather than failing the whole import.
                logger.warning(
                    f"Skipping a {serialized_view['type']} view of table "
                    f"{table_name} because that view type is not installed."
                )
                progress.increment(
                    state=f"{IMPORT_SERIALIZED_IMPORTING_TABLE_STRUCTURE}{table_name}"
                )
